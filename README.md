...
2. There will be a arrays used to store the game status. One of the significant statuses is the position of the player. We will use an char array to store it,  'C' represents the player ,the path walked by the player will also be recorded and shown. The status will be changed once the player moves.(Data structures for storing game status)
